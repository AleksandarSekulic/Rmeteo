pred.rfsi <- function (model, # RFSI model
  ...
  # prepare data
  data.prep <- data.prepare(data, data.staid.x.y.z,obs.col)
  ...
  newdata.prep <- data.prepare(newdata, newdata.staid.x.y.z)
  ...
  if (soil3d) { # soil - data.frame
    if (progress) print('Soil 3D process ...')
    newdata.cl <- "soil"
    if (!(any(class(newdata) %in% c(sp.class, "data.frame")))) { ##################################################### terra
      stop('The argument newdata must be sf, SpatVector, SpatRaster, or data.frame!')
    }
    if (is.na(newdata.staid.x.y.z[4])) {
      stop('The z (i.e. depth) element of argument newdata.staid.x.y.z must not be NA!')
    }
  } else { # not soil
    if (is.na(newdata.staid.x.y.z[4])) {
      if (progress) print('Spatial process ...')
  ...
  # check format
  if (newdata.cl %in% c("s", "soil") & output.format %in% c("sftime")) {
    stop('Argument output.format cannot be sftime for interpolation in spatial or soil domain!')
  }
  ...
  if (is.na(s.crs)) {
    ...
    if (progress) print('Using data projection CRS for Euclidean distances calculation. Do reprojection from:')
    if (progress) print(s.crs)
    if (progress) print('to:')
    if (progress) print(p.crs)
  ...
  if (is.na(newdata.s.crs)) {
    ...
    if (any(class(newdata.df) == "SpatRaster")) {
      # TO DO
      if (progress) print('Newdata is in SpatRaster format. No spatial transformation will be performed!')
      # if (progress) print('Using newdata target CRS for Euclidean distances calculation. Do reprojection from:')
      # if (progress) print(newdata.s.crs)
      # if (progress) print('to:')
      # if (progress) print(p.crs)
      # newdata.df <- terra::wrap
    } else {
      old.newdata.x.y <- newdata.staid.x.y.z[2:3]
      if (progress) print('Using newdata projection CRS for Euclidean distances calculation. Do reprojection from:')
      if (progress) print(newdata.s.crs)
      if (progress) print('to:')
      if (progress) print(p.crs)
      # reproject data coordinates
  ...
  if (soil3d) {
    data.depth.name <- names(data.df)[data.staid.x.y.z[4]]
    newdata.depth.name <- names(newdata.df)[newdata.staid.x.y.z[4]]
  }
  
  if (any(class(newdata.df) == "SpatRaster")) { # TERRA
    if (data.cl == "soil") { # soil 3D
      if (progress) print('Calculating distances to the nearest observations ...')
      dev_day_df <- data.df[, c(x.y, data.depth.name, obs.col.name)]
      day_df <- cbind(crds(newdata.df, na.rm=FALSE, df=T), values(newdata.df[[newdata.depth.name]]))
  ...
  if (output.format == "sftime") {
    ...
    if (newdata.cl == "st"){
      df <- result
      # vidi broj pred kolona i uradi for petlju - vrati listu stackova za quintile regression!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      names(df)[4] <- "time"
      unique_times <- unique(df$time)
      f <- function(x) terra::rast(df[df$time == x, c(2:3,6:length(df))], type="xyz", crs = final.crs)
