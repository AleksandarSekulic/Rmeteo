data.prepare <- function (data,
  ...
  if (any(class(data) == "SpatVector")) {
    ...
    if (is.null(data.staid.x.y.z)) {
      ...
      data.staid.x.y.z <- c(length(data.df),x.y.loc,NA)
    } else {
    ...
    data.df = data
    if (is.null(data.staid.x.y.z)) {
      # data.df$staid <- 1:ncell(data.df)
      data.staid.x.y.z <- c(NA,NA,NA,NA)
    } else {
      if (!is.numeric(data.staid.x.y.z)) {
        data.staid.x.y.z <- match(data.staid.x.y.z, names(data.df)) # sapply(data.staid.x.y.z, function(i) index(names(data))[names(data) == i])
      }
    ...
    if (is.null(data.staid.x.y.z)) {
      ...
      data.staid.x.y.z <- c(length(data.df),x.y.loc,length(data.df)-1)
    } else {
      if (!is.numeric(data.staid.x.y.z)) {
        data.staid.x.y.z <- match(data.staid.x.y.z, names(data.df)) # sapply(data.staid.x.y.z, function(i) index(names(data))[names(data) == i])
      }
      data.staid.x.y.z <- c(data.staid.x.y.z[1],x.y.loc,length(data.df)-1)
    }
    if (!is.na(st_crs(data))) {
      s.crs <- st_crs(data)
    }
